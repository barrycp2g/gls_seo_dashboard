...
// API service with dummy data (replace with actual API calls in production)
class APIService {
  static async fetchSEOData(): Promise<SEOData> {
    // Simulate API delay
    await new Promise(resolve => setTimeout(resolve, 1000));
    
    // Return comprehensive dummy data
    return {
      domainInfo: [
        {
          country_name: "Poland",
          country_code: "PL",
          domain_name: "https://gis-group.com/PL/pl/home/",
          total_keywords: 28531,
          total_keywords_brand: 3200,
          total_keywords_non_brand: 25331,
          avg_difficulty_brand: 25,
          avg_difficulty_non_brand: 65,
          nb_big_kw_opportunities: 156
        },
        {
          country_name: "Spain",
          country_code: "ES",
          domain_name: "https://gis-group.com/ES/es/home/",
          total_keywords: 15820,
          total_keywords_brand: 2100,
          total_keywords_non_brand: 13720,
          avg_difficulty_brand: 30,
          avg_difficulty_non_brand: 58,
          nb_big_kw_opportunities: 89
        },
        {
          country_name: "France",
          country_code: "FR",
          domain_name: "https://gis-group.com/FR/fr/home/",
          total_keywords: 19340,
          total_keywords_brand: 2450,
          total_keywords_non_brand: 16890,
          avg_difficulty_brand: 28,
          avg_difficulty_non_brand: 62,
          nb_big_kw_opportunities: 112
        },
        {
          country_name: "Germany",
          country_code: "DE",
          domain_name: "https://gis-group.com/DE/de/home/",
          total_keywords: 22890,
          total_keywords_brand: 2980,
          total_keywords_non_brand: 19910,
          avg_difficulty_brand: 32,
          avg_difficulty_non_brand: 70,
          nb_big_kw_opportunities: 134
        }
      ],
      keywordTypesBrand: generateKeywordTypes(true),
      keywordTypesNonBrand: generateKeywordTypes(false),
      longTailKeywords: generateLongTailKeywords(),
...
function generateKeywordTypes(isBrand: boolean): KeywordType[] {
  const countries = ['PL', 'ES', 'FR', 'DE'];
  const types = ['Informational', 'Commercial', 'Transactional', 'Navigational'];
  const colors = ['#3B82F6', '#10B981', '#F59E0B', '#EF4444'];
  const result: KeywordType[] = [];

  countries.forEach(country => {
    ['native', 'english'].forEach(lang => {
      const tag = `${country}-${lang === 'native' ? country : 'EN'}`;
      const baseValues = isBrand 
        ? [45, 30, 20, 5] 
        : [65, 20, 12, 3];
      ...
      types.forEach((type, i) => {
        ...
        result.push({
          ...
          name: type,
          tag,
          value: Math.max(0, baseValues[i] + variance),
          color: colors[i]
        });
      });
    });
...
function generateLongTailKeywords(): LongTailKeyword[] {
  const keywords: LongTailKeyword[] = [];
  const countries = [
    { code: 'PL', baseKeywords: ['wysyłka', 'paczka', 'kurier', 'dostawa'] },
    { code: 'ES', baseKeywords: ['envío', 'paquete', 'mensajero', 'entrega'] },
    { code: 'FR', baseKeywords: ['expédition', 'colis', 'courrier', 'livraison'] },
    { code: 'DE', baseKeywords: ['versand', 'paket', 'kurier', 'lieferung'] }
  ];

  countries.forEach(country => {
    ['native', 'english'].forEach(lang => {
      const tag = `${country.code}-${lang === 'native' ? country.code : 'EN'}`;
      const isEnglish = lang === 'english';
      
      for (let i = 0; i < 50; i++) {
        const baseWord = isEnglish 
          ? ['shipping', 'parcel', 'courier', 'delivery'][i % 4]
          : country.baseKeywords[i % 4];
        
        keywords.push({
          country_code: country.code,
          keyword: `${baseWord} ${isEnglish ? 'international' : 'międzynarodowy'} ${i + 1}`,
          tag,
          ...
          intent: ['Commercial', 'Informational', 'Transactional', 'Navigational'][Math.floor(Math.random() * 4)]
        });
      }
    });
  });
...
function generateCompetitors(): Competitor[] {
  const competitorsByCountry = {
    Poland: ['dpd.com', 'ups.com', 'fedex.com', 'inpost.pl', 'pocztex.pl'],
    Spain: ['correos.es', 'mrw.es', 'seur.com', 'nacex.es', 'gls-spain.es'],
    France: ['laposte.fr', 'chronopost.fr', 'colissimo.fr', 'dpd.fr', 'tnt.fr'],
    Germany: ['dhl.de', 'hermes.de', 'dpd.de', 'gls-group.eu', 'ups.com']
  };

  const competitors: Competitor[] = [];

  Object.entries(competitorsByCountry).forEach(([country, domains]) => {
    domains.forEach(domain => {
      competitors.push({
        country_name: country,
        domain,
        competitor_relevance: parseFloat((Math.random() * 0.3 + 0.6).toFixed(2)),
        common_keywords: Math.floor(Math.random() * 300) + 100,
        organic_keywords: Math.floor(Math.random() * 2000) + 1000,
        organic_traffic: Math.floor(Math.random() * 50000) + 20000,
        organic_cost: Math.floor(Math.random() * 100000) + 50000,
        google_ads_keywords: Math.floor(Math.random() * 200) + 50
      });
    });
  });
...
const PieChart = ({ data }: { data: KeywordType[] }) => {
  ...
  // Calculate total to ensure we're working with 100%
  ...
  return (
    <svg width="100%" height="100%" viewBox={`0 0 ${size} ${size}`} className="max-w-full">
      {/* Add a circle background to ensure no gaps */}
...
const SEODashboard = () => {
  ...
  const getCurrentCountry = useCallback(() => 
    countries.find(c => c.code === selectedCountry), [selectedCountry]
  );
  ...
  const transformedData = useMemo(() => {
    ...
    return {
      domainInfo: data.domainInfo.find(d => d.country_code === selectedCountry) || null,
      brandKeywords: data.keywordTypesBrand.filter(k => k.country_code === selectedCountry && k.tag === currentTag),
      nonBrandKeywords: data.keywordTypesNonBrand.filter(k => k.country_code === selectedCountry && k.tag === currentTag),
      longTailKeywords: data.longTailKeywords.filter(k => k.country_code === selectedCountry && k.tag === currentTag),
      competitors: data.competitors.filter(c => c.country_name === currentCountry?.name)
    };
  }, [data, selectedCountry, getCurrentTag, getCurrentCountry]);
  ...
  return (
    <div className="min-h-screen bg-gray-50">
        ...
        <div className="max-w-7xl mx-auto px-3 md:px-6">
          <div className="flex overflow-x-auto scrollbar-hide space-x-1">
            {countries.map(country => (
              <button
                key={country.code}
                className={`flex-shrink-0 px-4 md:px-6 py-3 md:py-4 font-medium border-b-2 transition-all ${
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between space-y-3 sm:space-y-0">
            <div className="flex items-center space-x-3 md:space-x-4">
              ...
              <button
                ...
                className={`relative inline-block w-10 md:w-12 h-5 md:h-6 rounded-full transition-colors duration-200 ease-in-out ${
                  selectedLanguage === 'english' ? 'bg-blue-500' : 'bg-gray-300'
                }`}
              >
                <div className={`absolute left-0.5 md:left-1 top-0.5 md:top-1 w-4 h-4 bg-white rounded-full transition-transform duration-200 ease-in-out ${
                  selectedLanguage === 'english' ? 'transform translate-x-5 md:translate-x-6' : ''
                }`}></div>
              </button>
              ...
              <span className="text-xs text-gray-400 hidden sm:inline">
                • {transformedData.longTailKeywords.length} keywords
              </span>
            </div>
            <button
              onClick={() => loadData(true)}
              className="flex items-center justify-center sm:justify-start px-3 py-1.5 md:px-4 md:py-2 text-xs md:text-sm bg-gray-100 hover:bg-gray-200 rounded transition-colors"
            >
          ...
          <div className="bg-white p-4 md:p-6 rounded-lg shadow">
            ...
            <div className="grid grid-cols-2 gap-3 md:gap-4">
              <div className="text-center p-3 md:p-4 bg-blue-50 rounded-lg">
                ...
                <div className="text-sm md:text-base text-gray-600">Brand Keywords</div>
              ...
              <div className="text-center p-3 md:p-4 bg-yellow-50 rounded-lg">
                ...
                <div className="text-sm md:text-base text-gray-600">Non-Brand Keywords</div>
                <div className="text-xs text-gray-500 mt-1">
                  {transformedData.domainInfo && transformedData.domainInfo.total_keywords > 0
                ...
                <div className="mt-3 md:mt-4 grid grid-cols-2 gap-2 w-full">
                  {transformedData.brandKeywords.map((type, index) => (
                    <div key={index} className="flex items-center text-xs md:text-sm">
                      ...
                      ></div>
                      <span className="text-gray-600 truncate">{type.name}: {type.value}%</span>
                    </div>
                  ))}
                </div>
              </div>
            ...
            <div className="block md:hidden">
              {competitorsPagination.paginatedData.map((competitor, index) => (
                <div key={index} className="p-4 border-b hover:bg-gray-50">
                  <div className="font-medium text-gray-900 mb-2">{competitor.domain}</div>
              ...
              <table className="min-w-full divide-y divide-gray-200">
                <thead className="bg-gray-50">
                  <tr>
                    ...
                    <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Paid Keywords</th>
                  </tr>
                </thead>
                <tbody className="bg-white divide-y divide-gray-200">
                  {competitorsPagination.paginatedData.map((competitor, index) => (
                    <tr key={index} className="hover:bg-gray-50 transition-colors">
                      <td className="px-6 py-4 text-sm font-medium text-gray-900">{competitor.domain}</td>
                    ...
                    <div>
                      ...
                      <div>
                        <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium ${
                          ...
                          keyword.intent === 'Transactional' ? 'bg-yellow-100 text-yellow-800' :
      ...
      <div className="bg-gray-800 text-white p-6 text-center">
        <p className="text-sm">© BC 2025 Parcel2Go × GLS - Multi-Country SEO Dashboard</p>
